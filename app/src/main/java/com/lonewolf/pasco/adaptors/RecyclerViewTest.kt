...
class RecyclerViewTest(context : Context, arrayList: ArrayList<HashMap<String, String>>) : RecyclerView.Adapter<RecyclerViewTest.MyHolder>() {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): RecyclerViewTest.MyHolder {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.layout_test, parent, false)

        return MyHolder(view)
    }

    override fun onBindViewHolder(holder: RecyclerViewTest.MyHolder, position: Int) {

        val hash = arrayList[position]
        val pos = ShortCut_To.numberToPosition(position + 1)
        holder.num.text = pos
        holder.date.text = hash["date"]
        holder.time.text = hash["time"]
